...
export default function TestCourierPage() {
    ...
    // mark-delivery-failed: требует reason
    ...
    const loadAvailable = useCallback(async () => {
        ...
        try {
            const r = await fetch('/api/courier/orders/available');
    ...
    const loadMyOrders = useCallback(async () => {
        ...
        try {
            const r = await fetch('/api/courier/orders/me');
            if (!r.ok) throw new Error((await r.json()).message ?? `HTTP ${r.status}`);
    ...
    async function doAction(orderId: string, label: string, endpoint: string, body?: object) {
    ...
    return (
        <div style={{ padding: 40, maxWidth: 960, fontFamily: 'monospace' }}>
            <h1 style={{ marginBottom: 6 }}>Courier · Тестирование</h1>
            <p style={{ color: '#888', fontSize: 12, marginTop: 0, marginBottom: 28 }}>
                <a href="/test-ops" style={{ color: '#0070f3' }}>← Хаб</a>
                {' · '}Роли: <strong>COURIER</strong>, ADMIN
                {' · '}Аккаунт: <code>courier@example.com</code>
            </p>
                        ...
                        <span style={{ fontSize: 13 }}>
                            ...
                            {session.role !== 'COURIER' && session.role !== 'ADMIN' && (
                                <span style={{ color: '#c00', marginLeft: 12 }}>
                                    ⚠ Роль {session.role} заблокирована для courier-маршрутов
                                </span>
                            )}
                        </span>
                ...
                {(authStep === 'unauth' || authStep === 'code-sent') && (
                    <div>
                        <p style={{ fontSize: 12, color: '#666', marginTop: 0, marginBottom: 10 }}>
                            Courier: <code>courier@example.com</code>
            ...
            {/* ── Результат действия ── */}
            {authStep === 'authed' && actionResult && <div style={successBox}>{actionResult}</div>}
            {authStep === 'authed' && actionError  && <div style={errorBox}>{actionError}</div>}
                ...
                <section style={sectionStyle}>
                    <div style={sectionFlex}>
                        <h2 style={{ ...sHead, marginBottom: 0 }}>
                            Доступные заказы (DELIVERY / DELIVERY_ASSIGNED)
                    ...
                    {!availLoading && available.length === 0 && !availError && (
                        <p style={{ ...muted, marginBottom: 0 }}>Нет доступных доставок</p>
                    )}
                            ...
                            actions={(o) => [
                                {
                                    label: 'Claim',
                                    style: btnPrimary,
                                    onClick: () => doAction(o.id, 'claim', `/api/courier/orders/${o.id}/claim`),
                                },
                            ]}
                        />
                    )}
            ...
            {authStep === 'authed' && (
                <section style={sectionStyle}>
                    ...
                    {!myLoading && myOrders.length === 0 && !myError && (
                        <p style={{ ...muted, marginBottom: 0 }}>Нет назначенных доставок</p>
                    )}
                            ...
                            actions={(o) => [
                                {
                                    label: 'Release',
                                    style: btnSecondary,
                                    onClick: () => doAction(o.id, 'release', `/api/courier/orders/${o.id}/release`),
                                },
                                ...(o.state === 'DELIVERY' || o.state === 'DELIVERY_ASSIGNED' ? [{
                                    label: 'Start Delivery',
                                    style: btnSuccess,
                                    onClick: () => doAction(o.id, 'start-delivery', `/api/courier/orders/${o.id}/start-delivery`),
                                }] : []),
                                ...(o.state === 'OUT_FOR_DELIVERY' ? [
                                    {
                                        label: 'Confirm Delivered',
                                        style: btnSuccess,
                                        onClick: () => doAction(o.id, 'confirm-delivered', `/api/courier/orders/${o.id}/confirm-delivered`),
                                    },
                                    {
                                        label: 'Mark Failed',
                                        style: btnDanger,
                                        onClick: () => doAction(
                                            ...
                                            `/api/courier/orders/${o.id}/mark-delivery-failed`,
                                            { reason: failReason[o.id] || 'Ошибка доставки' },
                                        ),
                                    },
                                ] : []),
                            ]}
            ...
            {authStep === 'authed' && (
                <section style={{ ...sectionStyle, background: '#f0fff8', borderColor: '#b7ebc5' }}>
                    ...
                    <ol style={{ fontSize: 13, margin: 0, paddingLeft: 20, lineHeight: 2 }}>
                        <li>COURIER claim/release только в разрешённом окне (DELIVERY / DELIVERY_ASSIGNED)</li>
                        <li>Повторный Claim тем же COURIER — идемпотентен (OK)</li>
                        <li>Параллельный Claim другим пользователем — ожидается <code>409 already claimed</code></li>
                        <li>Нецелевые роли (CUSTOMER, PICKER) — <code>403 Forbidden</code>
...
function CourierOrderTable({ orders, actions, actionLoading, failReason, setFailReason }: {
    orders: CourierOrder[];
    actions: (o: CourierOrder) => OrderAction[];
    actionLoading: string | null;
    failReason: Record<string, string>;
    setFailReason: React.Dispatch<React.SetStateAction<Record<string, string>>>;
}) {
    return (
            ...
            <tbody>
                {orders.map(o => (
                    <tr key={o.id} style={{ borderBottom: '1px solid #eee' }}>
                        ...
                        <td style={{ padding: '7px 8px', color: '#555', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {o.address ?? '—'}
                        </td>
                            ...
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                                {o.state === 'OUT_FOR_DELIVERY' && (
                                    <input
                                        ...
                                        value={failReason[o.id] ?? ''}
                                        onChange={ev => setFailReason(prev => ({ ...prev, [o.id]: ev.target.value }))}
                                        style={{ ...inputStyle, width: 160, padding: '3px 6px', fontSize: 11 }}
                                    />
                                )}
                                {actions(o).map(a => (
